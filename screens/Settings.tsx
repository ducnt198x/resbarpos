...
import { Settings as SettingsIcon, Globe, Moon, Printer, Wifi, Lock, LogOut, ChevronRight, Save, X, Check, AlertCircle, Loader2, DollarSign, Sun, Wallet, ShieldAlert, Briefcase, User, Smartphone, FileText } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ onLogout }) => {
  ...
  const [printConfig, setPrintConfig] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : { method: 'browser', paperSize: '80mm' };
  });
    ...
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden relative transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        <div className="max-w-4xl mx-auto flex flex-col gap-8 pb-12">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-2">
                        <label className="text-xs font-bold text-secondary uppercase tracking-wider flex items-center gap-2">
                          <FileText size={14} /> Paper Size
                        </label>
                        <div className="grid grid-cols-2 gap-2">
                          <button 
                            onClick={() => setPrintConfig({ ...printConfig, paperSize: '58mm' })}
                            className={`py-2 px-3 rounded-lg border font-bold text-xs transition-all ${printConfig.paperSize === '58mm' ? 'bg-primary text-background border-primary shadow-sm' : 'bg-background border-border text-secondary hover:border-primary/50'}`}
                          >
                            58mm (Small)
                          </button>
                          <button 
                            onClick={() => setPrintConfig({ ...printConfig, paperSize: '80mm' })}
                            className={`py-2 px-3 rounded-lg border font-bold text-xs transition-all ${printConfig.paperSize === '80mm' ? 'bg-primary text-background border-primary shadow-sm' : 'bg-background border-border text-secondary hover:border-primary/50'}`}
                          >
                            80mm (Large)
                          </button>
                        </div>
                        <p className="text-[10px] text-secondary italic mt-1">* Adjusts widths to prevent scaling down on mobile devices.</p>
                      </div>
                    </div>
          ...
          <div className="text-center pt-4"><p className="text-xs text-secondary font-medium">App Version: 1.0.8 (Thermal Optimized)</p></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface border border-border rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSavePassword} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex gap-3"><button type="button" onClick={() => setShowPasswordModal(false)} className="flex-1 py-3 rounded-lg border border-border text-text-main font-bold text-sm hover:bg-border transition-colors">{t('Cancel')}</button><button type="submit" disabled={isSavingPassword} className="flex-1 py-3 rounded-lg bg-primary text-background font-bold text-sm hover:bg-primary-hover transition-colors shadow-lg shadow-primary/20 flex items-center justify-center gap-2">{isSavingPassword ? <Loader2 className="animate-spin" /> : <Save size={18} />}{t('Update')}</button></div>
            </form>
          </div>
        </div>
      )}
